...
impl Brain {
    ...
    fn count_neigh(&self, i: usize, j: usize) -> usize {
        let mut res = 0;
        if self.field[self.index_move(i, j, -1, 0)].is_firing() { res += 1; }
        if self.field[self.index_move(i, j, -1, -1)].is_firing() { res += 1; }
        if self.field[self.index_move(i, j, -1, 1)].is_firing() { res += 1; }
        if self.field[self.index_move(i, j, 1, 0)].is_firing() { res += 1; }
        if self.field[self.index_move(i, j, 1, -1)].is_firing() { res += 1; }
        if self.field[self.index_move(i, j, 1, 1)].is_firing() { res += 1; }
        if self.field[self.index_move(i, j, 0, -1)].is_firing() { res += 1; }
        if self.field[self.index_move(i, j, 0, 1)].is_firing() { res += 1; }
        res
    }
